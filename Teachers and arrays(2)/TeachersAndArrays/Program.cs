    ...
    public class Program
    {
        ...
        private static void RunArrayProcessing()
        {
            try
            {
                ...
                var aarayA = new ArrayManager();
                Console.WriteLine(AppConstants.InputA);
                aarayA.ReadFromInput();

                var arrayB = new ArrayManager();
                Console.WriteLine(AppConstants.InputB);
                arrayB.ReadFromInput();

                int[] arrayC = ArrayProcessor.GenerateArrayC(aarayA, arrayB);
                Console.WriteLine($"{AppConstants.GeneratedC}" +
                    $"{string.Join(" ", arrayC)}");
            }
            catch (Exception ex)
            {
        ...
        private static void RunUniversityTeachers()
        {
            try
            {
                ...
                var teachers = UniversityDataProcessor.inputTeachers()
                    .ToList();
