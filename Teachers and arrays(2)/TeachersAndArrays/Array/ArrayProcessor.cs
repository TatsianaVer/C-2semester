...
namespace Arrays
{
    public static class ArrayProcessor
    {
        public static int[] GenerateArrayC(ArrayManager sourceA, ArrayManager sourceB)
        {
            ...
            if (AppConstants.ElementIndex < 0 
                || AppConstants.ElementIndex >= sourceA.Data.Length)
            {
                throw new IndexOutOfRangeException(AppConstants.IndexOutOfRangeMessage);
            }

            var segmentA = sourceA.Data.Skip(lastMinIndex + 1)
                .Take(AppConstants.ElementIndex - lastMinIndex)
                .ToArray();

            return segmentB.Concat(segmentA)
                .ToArray();
        }
    }
}
