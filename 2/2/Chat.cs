...
namespace Chat
{
    public class Chat
    {
        ...
        public string GetChatInfo() =>
            $"{ChatConstants.Chat}: {ChatName}, {ChatConstants.Users}: {UsersCount}, {ChatConstants.Messages}: {MessagesCount}, " +
            $"{ChatConstants.Participants}: {(UsersCount > 0 ? _users.ToString() : ChatConstants.None)}";
    }
}
